...
Public.on_load = function()
    minesweeper = storage.minesweeper
end
...
local function update_rendering(cell, position)
    ...
    cell[3] = rendering.draw_rectangle {
        width = 2,
        filled = false,
        surface = surface,
        left_top = position,
...
local function on_player_changed_position(event)
    ...
    if controllers[player.controller_type] then
        return
    end
    if not Functions.is_minefield_tile(player.physical_position) then
        return
    end
    local cell_position = Functions.position_to_cell_position(player.physical_position)
...
Public.on_init = function()
    storage.minesweeper = minesweeper
