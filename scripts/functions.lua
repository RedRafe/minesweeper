...
local Get_noise = require 'scripts.get_noise'
...
local function unstuck_players_around_position(surface, position)
    ...
    for _, character in pairs(characters) do
        if character.player then
            ...
            local p = surface.find_non_colliding_position('character', player.physical_position, 32, 0.5)
            if not p then
                return
            end
...
function Public.kaboom(position)
    ...
    local count = surface.count_entities_filtered({
        ...
        area = { { position.x - 4, position.y - 4 }, { position.x + 4, position.y + 4 } },
        limit = 1,
    })
    if count > 0 then
        return
    ...
    if search_cell then
        for x = 0, 1, 1 do
            for y = 0, 1, 1 do
                ...
                if tile.name == 'nuclear-ground' then
                    return true
                end
...
function Public.is_spawn(position)
    if math.abs(position.x) > safe_zone_radius then
        return false
    end
    if math.abs(position.y) > safe_zone_radius then
        return false
    end
...
function Public.get_terrain_tile(surface, position)
    if Public.is_spawn(position) then
    ...
    if noise_2 > 0 then
        return 'sand-' .. math.floor((noise_2 * 10) % 3 + 1)
    end
...
function Public.disarm_reward(position)
    ...
    if math.random(1, 3) ~= 1 or tile_name == 'water-shallow' then
        return
    end
...
function Public.uncover_terrain(position)
    ...
    for x = 0, 1, 1 do
        for y = 0, 1, 1 do
            ...
            if mineable_tile_name and tile_name ~= 'water-shallow' then
                surface.set_tiles({ { name = mineable_tile_name, position = p } }, true)
            end
        end
    end
end
...
function Public.shuffle_table(t, rng)
